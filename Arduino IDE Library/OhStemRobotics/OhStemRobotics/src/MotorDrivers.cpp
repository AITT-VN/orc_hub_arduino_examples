...
void MotorDriverV1::setMotors(uint8_t motors, int speed) {
  ...
  // V1 expects one packet per motor channel, so expand the bitmask manually.
...
bool MotorDriverV2::begin() {
  ...
  // A matching WHO_AM_I value confirms the I2C device is the expected driver board.
...
bool MotorDriverV2::readBytes(uint8_t reg, uint8_t* buffer, size_t len) {
  // Use a repeated-start transaction so the target register can be read directly.
  wire_->beginTransmission(address_);
  wire_->write(reg);
...
void MotorDriverV2::setLocalMotor(uint8_t index, int speed) {
  ...
  // These optional pins let M3/M4 be mirrored to local H-bridge outputs.
  if (speed > 0) {
    analogWrite(in1, pwm);
    analogWrite(in2, 0);
  } else if (speed < 0) {
...
void MotorDriverV2::setMotors(uint8_t motors, int speed) {
  speed = clampValue(speed, -100, 100);
  // The V2 board accepts a compact "motor mask + speed" command packet.
...
void MotorDriverV2::setServo(uint8_t index, int angle, int maxAngle) {
  ...
  // Normalize custom servo ranges to the 0..180 scale used by the hardware.
...
int32_t MotorDriverV2::getEncoder(uint8_t motors) {
  ...
  // E1 and E2 are stored in one contiguous register block.
...
int16_t MotorDriverV2::getSpeed(uint8_t motors) {
  ...
  // Speed registers follow the same E1/E2 packing convention as encoders.
