...
DriveBase::DriveBase(DriveMode driveMode, DCMotorBase* m1, DCMotorBase* m2,
  ...
  // Left-side motors are reversed here so higher-level commands can assume
  // that positive speed always means "move the robot forward".
  // When using DriveBase, do not also pass reversed=true for the left motors
  // unless your chassis wiring is intentionally different from the default.
  if (m1 != nullptr) {
    m1->reverse();
    left_[leftCount_++] = m1;
...
void DriveBase::straight(float speedValue, float amount, MoveUnit unit,
  ...
  const uint32_t startMs = millis();
  // Convert the requested travel unit into a single target value.
  if (unit == CM) {
  ...
  while (true) {
    tickAngleSensor();
    // For time-based moves, progress is measured in ms. For distance-based moves,
    // progress comes from the attached wheel encoders.
    ...
    if (!((unit == SECOND && amount < 2.0f) || (unit == CM && amount < 10.0f) ||
      ...
      // Ramp the speed up and down on longer moves to reduce overshoot.
      expectedSpeed =
...
void DriveBase::turn(float steering, float amount, MoveUnit unit,
  ...
  if (unit == DEGREE) {
    if (useGyro_ && angleSensor_ != nullptr) {
      // Gyro turns measure progress directly from the heading estimate.
      target = min(fabs(amount), 359.0f);
      angleSensor_->reset();
    } else {
      // Without a gyro, estimate turn progress from wheel travel along the arc.
  ...
  while (true) {
    ...
    if (!((unit == SECOND && amount < 1.0f) || (unit == DEGREE && amount < 45.0f))) {
      // Apply the same ramp profile used by straight moves for smoother turns.
...
void DriveBase::run(Direction dir, float speedValue) {
  ...
  if (driveMode_ == MODE_MECANUM) {
    // Each row describes how one direction maps to the four wheel speeds.
...
float DriveBase::distance() {
  if (leftEncoder_ == nullptr || rightEncoder_ == nullptr) {
    return 0.0f;
  }
...
void DriveBase::updateTeleop(const GamepadState& state, uint8_t accelSteps) {
  ...
  // Prefer the analog stick when it is clearly displaced, then fall back to d-pad
  // or action buttons.
  ...
  if (dir != lastTeleopDir_) {
    // Reset the acceleration ramp when the user changes direction.
  ...
  if (activeButton != GamepadButton::Count) {
    ...
    // Button callbacks fire only once per press instead of repeating every loop.
...
float DriveBase::calcSpeed(float speedValue, float distanceValue, float drivenDistance,
  ...
  // Use a simple trapezoidal profile: accelerate, cruise, then decelerate.
...
void DriveBase::calibSpeed(float speedValue, float& left, float& right) {
  ...
  if (useGyro_) {
    ...
    // Without a gyro, keep both sides balanced by comparing encoder progress.
...
void DriveBase::followLine(bool backward, LineState lineState) {
  ...
  // Map each sensor state to a corrective motion. Larger offsets produce
...
void DriveBase::followLineUntilCross(StopAction thenAction) {
  ...
  // State 1 waits until the robot leaves the starting cross.
  // State 2 looks for the next cross and stops on the second detection.
  int status = 1;
  int count = 0;
  while (true) {
    const LineState lineState = lineSensor_->check();
